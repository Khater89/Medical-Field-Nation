  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      ...
      <main className="container max-w-4xl py-6 flex-1">
        ...
        <h1 className="text-2xl font-bold mb-4">{t("mp.cart.title")}</h1>

        {items.length === 0 ? (
          <Card className="p-12 text-center">
            <ShoppingBag className="h-12 w-12 text-muted-foreground mx-auto mb-3" />
            <p className="text-muted-foreground mb-4">{t("mp.cart.empty")}</p>
            <Link to="/marketplace"><Button>{t("mp.cart.browse")}</Button></Link>
          </Card>
        ) : (
              ...
              {items.map((item) => (
                <Card key={item.product_id} className="p-3 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">{item.price.toFixed(2)} {currency}</div>
                  </div>
